        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-24 items-center">
          <div>
            <h2 className="text-5xl md:text-6xl font-black mb-10 leading-[0.9] tracking-tighter text-white">
              {t('sorglosPaket.title')} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">{t('sorglosPaket.titleHighlight')}</span>
            </h2>
            <p className="text-2xl text-gray-400 mb-12 font-light">
              {t('sorglosPaket.description')}
            </p>
            <ul className="grid gap-6 mb-12">
              {t('sorglosPaket.items').map((item: string, i: number) => (
          ...
          <div className="relative">
            ...
            <div className="bg-white/5 backdrop-blur-3xl p-12 rounded-[40px] border border-white/10 shadow-2xl relative overflow-hidden group">
              ...
              <div className="text-center relative z-10">
                ...
                <h3 className="text-3xl font-black text-white mb-6">{t('sorglosPaket.cardTitle')}</h3>
                <p className="text-cyan-400 mb-10 italic text-lg leading-relaxed font-light">"{t('sorglosPaket.cardQuote')}"</p>
                <div className="inline-block px-8 py-4 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-md">
                  <span className="text-4xl font-black text-white">{t('sorglosPaket.cardInclusive')}</span>
                </div>
                <p className="text-sm text-gray-500 mt-6 uppercase tracking-widest font-bold">{t('sorglosPaket.cardSubtext')}</p>
              </div>
            </div>
          </div>
        </div>
